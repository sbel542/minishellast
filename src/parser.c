...
t_ast_node	*parse_redirection(t_token **tokens, int *token_pos)
{
	...
	if (tokens[*token_pos]->type == TOKEN_REDIRECT_IN)
		...
		return (NULL);
	redir_node->value = strdup(tokens[*token_pos]->value);
	...
	if (tokens[*token_pos] && tokens[*token_pos]->type == TOKEN_WORD)
	{
		...
		redir_node->right->value = strdup(tokens[*token_pos]->value);
...
t_ast_node	*parse_command(t_token **tokens, int *token_pos)
{
	...
	command_node->value = strdup(tokens[*token_pos]->value);
	...
	while (tokens[*token_pos])
	{
		if (tokens[*token_pos]->type == TOKEN_WORD)
		{
			...
			arg_node->value = strdup(tokens[*token_pos]->value);
			arg_node->left = NULL;
			arg_node->right = NULL;
			arg_node->next = NULL;
...
t_ast_node	*parse_pipeline(t_token **tokens, int *token_pos)
{
	...
	while (tokens[*token_pos] && tokens[*token_pos]->type == TOKEN_PIPE)
	{
		...
		pipe_node->next = NULL;
		(*token_pos)++;
		pipe_node->right = parse_command(tokens, token_pos);
		if (!pipe_node->right)
		{
			free_ast(pipe_node);
			return (NULL);
